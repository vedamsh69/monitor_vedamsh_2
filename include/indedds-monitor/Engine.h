...
#include <indedds-monitor/model/info/InfoModel.h>
...
#include <indedds-monitor/model/tree/StatusTreeModel.h>
...
#include <indedds-monitor/LogModel.h>
...
TopicIDLStruct * getTopicIDLModel();

/**
 * @brief Get the synchronous backend connection
 * @return Pointer to SyncBackendConnection, or nullptr if not initialized
 */
backend::SyncBackendConnection* get_backend_connection();
...
LogModel dllogmodel;
   ...
   void setWindow(MainWindow *mainWindow)
    ...
    /*!
 * @brief Connect System Overview Model to Engine
 * @param model Pointer to SystemOverviewModel instance
 */
void setSystemOverviewModel(SystemOverviewModel* model);

/*!
 * @brief Update System Overview Panel with all discovered entities
 * Populates the model with hosts, participants, publishers, subscribers, etc.
 */
void updateSystemOverviewPanel();

/*!
 * @brief Notify System Overview of new entity discovery
 * @param entityId The ID of the newly discovered entity
 * @param entityKind The kind/type of the entity
 */
void notifySystemOverviewNewEntity(
    const backend::EntityId& entityId,
...
signals:
    ...
    void triggerCallbackProcessing();
    void triggerStatusCallbackProcessing();
    void engine_error(
...
public slots:                           
    ...
    //! Slot to receive entity updates from worker thread
    void onEntityUpdateReady(const backend::EntityUpdatePacket& packet);

    //! Slot to receive status updates from worker thread
    void onStatusUpdateReady(const backend::EntityId& entity_id, 
                            const backend::StatusKind& status_kind);

    void onWorkerCallbacksBatchDone();
    void onWorkerStatusBatchDone();
...
protected:
    ...
    std::atomic_bool callback_drain_scheduled_{false};
        std::atomic_bool status_drain_scheduled_{false};
    ...
    //! Worker thread for background callback processing
    QThread* callback_worker_thread_;

    //! Worker object that processes callbacks asynchronously
    backend::CallbackWorker* callback_worker_;

    //! Flag to enable/disable multi-threading (for gradual rollout)
...
//==============================================================================================
    ...
    QTimer system_overview_update_timer_;
    std::atomic<bool> system_overview_update_pending_{false};
