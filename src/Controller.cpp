...
void Controller::startPublisherWithDiscovery(const QString& topicName, int domainId)
{
    ...
    std::thread discovery_thread([this, topicName, domainId, key, subscriber, publisher]()
    {
        ...
        // Step 2 — Preferred path: initialize publisher from the exact
        ...
        // rebuilding a "similar" type from IDL text only.
        {
            ...
            if (discovered_type)
            {
                qDebug() << "[Controller] Initializing from discovered DynamicType:"
        ...
        // Step 3 — Fallback: build type from IDL text if DynamicType not available
        if (!publisherReady && !idlText.trimmed().isEmpty())
        {
            qDebug() << "[Controller] Discovered DynamicType unavailable — trying IDL text fallback";
