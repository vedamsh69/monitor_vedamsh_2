...
#include <indedds-monitor/model/dds/EndpointModelItem.h>
#include <indedds-monitor/model/dds/LocatorModelItem.h>
#include <indedds-monitor/model/dds/ParticipantModelItem.h>
#include <indedds-monitor/model/ListItem.h>
#include <indedds-monitor/model/ListModel.h>
#include <indedds-monitor/model/logical/DomainModelItem.h>
#include <indedds-monitor/model/logical/TopicModelItem.h>
#include <indedds-monitor/model/physical/HostModelItem.h>
#include <indedds-monitor/model/physical/ProcessModelItem.h>
#include <indedds-monitor/model/physical/UserModelItem.h>
...
namespace backend
{
    ...
    ListItem *SyncBackendConnection::create_host_data_(
        EntityId id)
    {
        qDebug() << "[DEBUG] Entering function create_host_data_";
        qDebug() << "Creating Host " << backend::backend_id_to_models_id(id);
        ListItem* item = new HostModelItem(id, get_info(id));
        qDebug() << "[DEBUG] Exiting function create_host_data_";
        return item;
    }
    ...
    ListItem *SyncBackendConnection::create_user_data_(
        EntityId id)
    {
        qDebug() << "[DEBUG] Entering function create_user_data_";
        qDebug() << "Creating User " << backend::backend_id_to_models_id(id);
        ListItem* item = new UserModelItem(id, get_info(id));
        qDebug() << "[DEBUG] Exiting function create_user_data_";
        return item;
    }
    ...
    ListItem *SyncBackendConnection::create_process_data_(
        EntityId id)
    {
        qDebug() << "[DEBUG] Entering function create_process_data_";
        qDebug() << "Creating Process " << backend::backend_id_to_models_id(id);
        ListItem* item = new ProcessModelItem(id, get_info(id));
        qDebug() << "[DEBUG] Exiting function create_process_data_";
        return item;
    }
    ...
    ListItem *SyncBackendConnection::create_domain_data_(
        EntityId id)
    {
        qDebug() << "[DEBUG] Entering function create_domain_data_";
        qDebug() << "Creating Domain " << backend::backend_id_to_models_id(id);
        ListItem* item = new DomainModelItem(id, get_info(id));
        qDebug() << "[DEBUG] Exiting function create_domain_data_";
        return item;
    }
    ...
    ListItem *SyncBackendConnection::create_topic_data_(
        EntityId id)
    {
        qDebug() << "[DEBUG] Entering function create_topic_data_";
        qDebug() << "Creating Topic " << backend::backend_id_to_models_id(id);
        ListItem* item = new TopicModelItem(id, get_info(id));
        qDebug() << "[DEBUG] Exiting function create_topic_data_";
        return item;
    }
    ...
    ListItem *SyncBackendConnection::create_participant_data_(
        backend::EntityId id)
    {
        qDebug() << "[DEBUG] Entering function create_participant_data_";
        qDebug() << "Creating Participant " << backend::backend_id_to_models_id(id);
        ListItem* item = new ParticipantModelItem(id, get_info(id));
        qDebug() << "[DEBUG] Exiting function create_participant_data_";
        return item;
    }
    ...
    ListItem *SyncBackendConnection::create_datawriter_data_(
        backend::EntityId id)
    {
        qDebug() << "[DEBUG] Entering function create_datawriter_data_";
        qDebug() << "Creating DataWriter " << backend::backend_id_to_models_id(id);
        ListItem* item = new EndpointModelItem(id, get_info(id), EntityKind::DATAWRITER);
        qDebug() << "[DEBUG] Exiting function create_datawriter_data_";
        return item;
    }
    ...
    ListItem *SyncBackendConnection::create_datareader_data_(
        backend::EntityId id)
    {
        qDebug() << "[DEBUG] Entering function create_datareader_data_";
        qDebug() << "Creating DataReader " << backend::backend_id_to_models_id(id);
        ListItem* item = new EndpointModelItem(id, get_info(id), EntityKind::DATAREADER);
        qDebug() << "[DEBUG] Exiting function create_datareader_data_";
        return item;
    }
    ...
    ListItem *SyncBackendConnection::create_locator_data_(
        backend::EntityId id)
    {
        qDebug() << "[DEBUG] Entering function create_locator_data_";
        qDebug() << "Creating Locator " << backend::backend_id_to_models_id(id);
        ListItem* item = new LocatorModelItem(id, get_info(id));
        qDebug() << "[DEBUG] Exiting function create_locator_data_";
        return item;
    }
    ...
    bool SyncBackendConnection::update_model_(
        ...
        bool metatraffic_visible)
    {
        qDebug() << "[DEBUG] Entering function update_model_";
        bool changed = false;

        // For each User get all processes
        for (auto subentity_id : get_entities(type, id))
        {
            qDebug() << "[DEBUG] update_model_: Processing subentity_id " << backend::backend_id_to_models_id(subentity_id);
            ...
            if (index == -1)
            {
                qDebug() << "[DEBUG] update_model_: subentity_id does not exist, will create.";
                // Only create the new entity if is alive or inactive are visible,
                // and if is metatraffic or metatraffic is visible
                if ((inactive_visible || get_alive(subentity_id)) && (metatraffic_visible || !is_metatraffic(subentity_id)))
                {
            ...
            else if ((!inactive_visible && !get_alive(subentity_id)) || (!metatraffic_visible && is_metatraffic(subentity_id)))
            {
                qDebug() << "[DEBUG] update_model_: subentity_id is inactive/metatraffic, will remove.";
            ...
            {
                qDebug() << "[DEBUG] update_model_: subentity_id exists, will update.";
                models::ListItem *subentity_item = model->at(index);
                changed = update_item_(subentity_item, update_function, inactive_visible, metatraffic_visible) || changed;
            }
        }
        qDebug() << "[DEBUG] Exiting function update_model_";
        return changed;
    }
        ...
        backend::EntityId id)
    {
        ...
        std::vector<std::pair<DataKind, StatisticKind>> configurations = {
            ...
            //=====================================================================
            std::make_pair(DataKind::HEARTBEAT_COUNT_NEW, StatisticKind::SUM),
            //=====================================================================
        ...
        for (auto configuration : configurations)
        {
            ...
            if (!data.empty() && !std::isnan(data[0].second))
            {
                ...
                summary[backend::data_kind_to_string(configuration.first)]
                       [backend::statistic_kind_to_string(configuration.second)] = utils::double_to_string(data[0].second) + " [" + get_data_kind_units(configuration.first) + "]";
            }
        }
    ...
    //==============================================================================================
    std::string SyncBackendConnection::get_host_for_process(
        models::ListModel *physical_model,
        EntityId process_id)

    {
        ListModel *host_model = get_model_(physical_model, process_id, EntityKind::HOST);
        if (host_model == nullptr)
        {
            std::cout << "Error getting host model for process " << process_id.value() << std::endl;
            return "";
        }
        std::cout << "Got Host model =========================== " <<std::endl;


        ListModel *user_model = get_model_(host_model, process_id, EntityKind::USER);
        if (user_model == nullptr)
        {
            std::cout << "Error getting user model for process " << process_id.value() << std::endl;
                        return "";

        }

        std::cout << "Got user model =========================== " <<std::endl;

        int index = user_model->rowIndexFromId(process_id);

         if (index != -1)
         {
        ListItem *item = user_model->at(index);

           EntityId host_id = item->get_entity_id();
        std::cout << "Process Id (sync backend connection) ====================" << process_id << std::endl;
        std::cout << "Host Id (sync backend connection)  ======================" << host_id << std::endl;

        return get_alias(host_id);
        ...
        DataKind data_kind)
    {
        ...
        case DataKind::HEARTBEAT_COUNT:
            return "count";
            //====================================================================================
        case DataKind::HEARTBEAT_COUNT_NEW:
            return "count";
            //===================================================================================
    ...
    {
        return std::vector<std::string>({"MEAN",
                                         "STANDARD_DEVIATION",
                                         "MAX",
                                         "MIN",
                                         "MEDIAN",
    ...
    {
        return std::vector<std::string>({"FASTDDS_LATENCY",
