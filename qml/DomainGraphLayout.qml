...
Item
{
    ...
    function filter_model_by_topic (topic_id)
    {
        ...
        console.log("Domain view graph:", model_string)
        ...
        if (model_string.length !== 0 && model_string !== "null")
        {
            ...
            console.log("Parsed model:", JSON.stringify(new_model, null, 2))
            ...
            if (new_model["domain"] == domain_id)
            {
                ...
                console.log("Is metatraffic visible:", is_metatraffic_visible_)
                ...
                for (var topic in new_model["topics"])
                {
                    ...
                    console.log("Topic:", topic, "Metatraffic:", metatraffic_)
                    if (metatraffic_ != true || is_metatraffic_visible_)
                    {
                        if (filtered_topics_.length > 0)
                        {
                            for (var i = 0; i < filtered_topics_.length; i++)
                            {
                                if (filtered_topics_[i] == topic)
                                {
                                    ...
                                    console.log("Added filtered topic:", new_topics[new_topics.length - 1])
                                }
                            }
                        }
                        else
                        {
                            ...
                            endpoints_per_topic[topic] = []
                            console.log("Added topic:", JSON.stringify(new_topics[new_topics.length - 1], null, 2))
                        }
                    }
                }
                var accum_y = 0
                ...
                for (var host in new_model["hosts"])
                {
                    ...
                    console.log("Host:", host, "Metatraffic:", metatraffic_)
                    if (metatraffic_ != true || is_metatraffic_visible_)
                    {
                        ...
                        for (var user in new_model["hosts"][host]["users"])
                        {
                            ...
                            console.log("User:", user, "Metatraffic:", metatraffic_)
                            if (metatraffic_ != true || is_metatraffic_visible_)
                            {
                                accum_y += label_height_ + elements_spacing_
                                var new_processes = []
                                ...
                                for (var process in new_model["hosts"][host]["users"][user]["processes"])
                                {
                                    ...
                                    console.log("Process:", process, "Metatraffic:", metatraffic_)
                                    if (metatraffic_ != true || is_metatraffic_visible_)
                                    {
                                        accum_y += label_height_ + elements_spacing_
                                        ...
                                        for (var participant in new_model["hosts"][host]["users"][user]["processes"][process]["participants"])
                                        {
                                            ...
                                            console.log("Participant:", participant, "Metatraffic:", metatraffic_)
                                            if (metatraffic_ != true || is_metatraffic_visible_)
                                            {
                                                accum_y += label_height_ + elements_spacing_
                                                var new_endpoints = []
                                                for (var endpoint in new_model["hosts"][host]["users"][user]["processes"][process]["participants"][participant]["endpoints"])
                                                {
                                                    ...
                                                    console.log("Endpoint:", endpoint, "Metatraffic:", metatraffic_)
                                                    if (metatraffic_ != true || is_metatraffic_visible_)
                                                    {
                                                                                           ...
                                                                                           && filtered_topics_.includes(new_model["hosts"][host]["users"][user]["processes"][process]["participants"][participant]["endpoints"][endpoint]["topic"])))
                                                        {
                                                            ...
                                                            console.log("Added endpoint:", JSON.stringify(new_endpoints[new_endpoints.length - 1], null, 2))
                                                        }
                                                    }
                                                }
                                                if (!discard_participant)
                                                {
                                                    ...
                                                    console.log("Added participant:", JSON.stringify(new_participants[new_participants.length - 1], null, 2))
                                        ...
                                        if (!discard_process)
                                        {
                                            ...
                                            console.log("Added process:", JSON.stringify(new_processes[new_processes.length - 1], null, 2))
                                            accum_y += elements_spacing_
                                ...
                                if (!discard_user)
                                {
                                    ...
                                    console.log("Added user:", JSON.stringify(new_users[new_users.length - 1], null, 2))
                                    accum_y += elements_spacing_
                                    user_temp_y = accum_y
                                }
                                else
                        ...
                        if (!discard_host)
                        {
                            ...
                            console.log("Added host:", JSON.stringify(new_hosts[new_hosts.length - 1], null, 2))
                            accum_y += elements_spacing_
                            host_temp_y = accum_y
                        }
                        else
        ...
        if (filtered_topics_.length > 0)
        {
            if (filtered_topics_.length == 1)
            {
                ...
                console.log("Updated tab name:", topic_names[0] + " Topic View")
            }
            else
            {
                ...
                console.log("Updated tab name:", print_topic_names + " Topics View")
            }
        }
        else
        {
            domainGraphLayout.update_tab_name("Domain " + domain_id + " View", component_id)
            console.log("Updated tab name:", "Domain " + domain_id + " View")
        }
    ...
    function clear_graph()
    {
        ...
        entity_box_width_ = 0;
        console.log("Reset variables:", JSON.stringify({
                                                           topic_locations: topic_locations_,
                                                           endpoint_topic_connections: endpoint_topic_connections_,
                                                           endpoint_painted: endpoint_painted_,
                                                           topic_painted: topic_painted_,
                                                           pending_endpoints: pending_endpoints_,
                                                           pending_connections: pending_connections_,
                                                           endpoints_per_topic: endpoints_per_topic,
                                                           vertical_bar_position: vertical_bar.position,
                                                           horizontal_bar_position: horizontal_bar.position,
                                                           entity_box_width: entity_box_width_
                                                       }, null, 2))
        
        var mainSpaceChildrenDestroyed = 0
        for (var i = 0; i < mainSpace.children.length; i++)
        {
            if (mainSpace.children[i].left_margin != undefined)
            {
                mainSpace.children[i].destroy()
                mainSpaceChildrenDestroyed++
            }
        }
        console.log("Destroyed mainSpace children:", mainSpaceChildrenDestroyed)
        
        var topicConnectionsChildrenDestroyed = 0
        for (var i = 0; i < topic_connections.children.length; i++)
        {
            if (topic_connections.children[i].left_margin != undefined)
            {
                ...
                topicConnectionsChildrenDestroyed++
            }
        }
        console.log("Destroyed topic_connections children:", topicConnectionsChildrenDestroyed)
    ...
    {
        console.log("Checking if model contains entity. domainEntityId:", domainEntityId, "entityId:", entityId)
        ...
        if (domainEntityId != "")
        {
            ...
            {
                console.log("Entity does not belong to the current domain")
                return false
            }
        }
        ...
        {
            console.log("Entity found: domain")
            return true
        }
        ...
        for (var topic in model["topics"])
        {
            ...
            {
                console.log("Entity found: topic")
                return true
            }
        }
                ...
                for (var user in model["hosts"][host]["users"])
                {
                    ...
                    else
                    {
                        for (var process in model["hosts"][host]["users"][user]["processes"])
                        {
                            ...
                            {
                                console.log("Entity found: process")
                                return true
                            }
                            else
                            {
                                for (var participant in model["hosts"][host]["users"][user]["processes"][process]["participants"])
                                {
                                    ...
                                    {
                                        console.log("Entity found: participant")
                                        return true
                                    }
                                    else
                                    {
                                        for (var endpoint in model["hosts"][host]["users"][user]["processes"][process]["participants"][participant]["endpoints"])
                                        {
                                            ...
                                            {
                                                console.log("Entity found: endpoint")
                                                return true
                                            }
                                        }
                                    }
    ...
    {
        console.log("Topic visibility changed. topic_id:", topic_id, "inView:", inView)
        if (topic_id in endpoints_per_topic)
        {
            console.log("Number of endpoints for this topic:", endpoints_per_topic[topic_id].length)
            for (var i = 0; i < endpoints_per_topic[topic_id].length; i++)
            {
                console.log("Changing visibility for endpoint:", endpoints_per_topic[topic_id][i])
                domainGraphLayout.topic_visibility_changed_(endpoints_per_topic[topic_id][i], inView)
            }
        }
        else
        {
            console.log("No endpoints found for this topic")
        }
    }
    ...
    {
        console.log("Checking if connections are generated. topic_connections_generated:", domainGraphLayout.topic_connections_generated, "endpoint_connections_generated:", domainGraphLayout.endpoint_connections_generated)
        if (domainGraphLayout.topic_connections_generated && domainGraphLayout.endpoint_connections_generated)
        {
            console.log("All connections generated. Initializing graph...")
        ...
        {
            console.log("Not all connections are generated yet")
        }
    }
